...
// CHAT GPT SAID:
// However, there is one important conceptual issue here:
// this version uses bootsSymEncrypt() inside HE_decrypt(),
// which means the evaluator has access to he_sk.
// That defeats the normal hybrid-HE model. Kreyvium's actual HE_decrypt()
// never encrypts the input ciphertext using the secret HE key;
// the symmetric ciphertext is plaintext/public data and gets combined with 
// the already-HE-encrypted symmetric key.

// void MY_BASIC_ALGORITHM_TFHE::encrypt_key() {
...
void MY_BASIC_ALGORITHM_TFHE::encrypt_key()
{
    ...
    for (size_t i = 0; i < params.key_size_bits; i++) {
        uint8_t bit =
            (secret_key[i / 8] >> (7 - i % 8)) & 1;

        bootsSymEncrypt(
            &secret_key_encrypted[i],
            bit,
            he_sk
        );
...
// TFHECiphertextVec MY_BASIC_ALGORITHM_TFHE::HE_decrypt(std::vector<uint8_t>& ciphertexts,
//                                             size_t bits) {
//     TFHECiphertextVec out(bits, context);

//     for (size_t i = 0; i < bits; i++) {

//         int bit = (ciphertexts[i / 8] >> (7 - i % 8)) & 1;

//         // Encrypt the ordinary ciphertext bit
//         bootsSymEncrypt(&out[i], bit, he_sk);

//         // The single homomorphic operation
...
//   // for (size_t i = 0; i < MY_BASIC_ALGORITHM_PARAMS.key_size_bits; i++) {
//   //   lweCopy(inner_key[i],
//   //           &secret_key_encrypted[MY_BASIC_ALGORITHM_PARAMS.key_size_bits - 1 - i],
...
//   //   TFHECiphertext t4 = inner_key[MY_BASIC_ALGORITHM_PARAMS.key_size_bits - 1];
...
TFHECiphertextVec MY_BASIC_ALGORITHM_TFHE::HE_decrypt(
    std::vector<uint8_t>& ciphertexts,
    size_t bits)
{
    TFHECiphertextVec out(bits, context);
...
std::vector<uint8_t> MY_BASIC_ALGORITHM_TFHE::decrypt_result(
