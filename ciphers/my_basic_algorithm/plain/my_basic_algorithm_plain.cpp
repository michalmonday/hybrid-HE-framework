...
// std::vector<uint8_t> My_Basic_Algorithm::encrypt(std::vector<uint8_t> plaintext,
//                                        size_t bits) const {
...
std::vector<uint8_t> My_Basic_Algorithm::encrypt(
...
std::vector<uint8_t> My_Basic_Algorithm::decrypt(std::vector<uint8_t> ciphertext,
                                       size_t bits) const {
  // keyblock key = convert_key(secret_key);
  ...
  // return ks;
    ...
    // NOT(NOT(x)) = x
    return encrypt(ciphertext, bits);
}

void My_Basic_Algorithm::prep_one_block() const {}

std::vector<uint8_t> My_Basic_Algorithm::keystream(const keyblock& key, const ivblock& iv,
                                         size_t bits) const {
  ...
  //   bool t4 = inner_key[MY_BASIC_ALGORITHM_PARAMS.key_size_bits - 1];
...
void My_Basic_Algorithm::init(const keyblock& key, const ivblock& iv, stateblock& state,
  ...
  for (size_t i = 0; i < MY_BASIC_ALGORITHM_PARAMS.key_size_bits; i++) {
    inner_key[i] = key[MY_BASIC_ALGORITHM_PARAMS.key_size_bits - 1 - i];
  }
}
